        ...
        function parse() {
            ...
            if (tok == "[") {
                ...
                while (tok != "]") {
                    if (tok != "name")
                        err();
                    var prop = lezer_1.NodeProp[value], val = "";
                    if (!(prop instanceof lezer_1.NodeProp))
                        err();
                    next();
                    if (tok == "=") {
                        next();
...
function testTree(tree, expect, mayIgnore) {
    ...
    tree.iterate({
        enter: function (type, start) {
            ...
            if (next && next.matches(type)) {
                ...
                var after_1 = next ? next.name + (parent_1 == "tree" ? "" : " in " + parent_1) : "end of " + parent_1;
                throw new Error("Expected " + after_1 + ", got " + type.name + " at " + start + " \n" + tree);
            }
        },
        leave: function (type, start) {
            ...
            var last = stack.length - 1, index = pos[last], seq = stack[last];
            if (index < seq.length)
                throw new Error("Unexpected end of " + type.name + ". Expected " + seq.slice(index).map(function (s) { return s.name; }).join(", ") + " at " + start + "\n" + tree);
            pos.pop();
            stack.pop();
            pos[last - 1]++;
        }
    ...
    if (pos[0] != specs.length)
        throw new Error("Unexpected end of tree. Expected " + stack[0].slice(pos[0]).map(function (s) { return s.name; }).join(", ") + " at " + tree.length + "\n" + tree);
}
    ...
    var _loop_1 = function () {
        ...
        tests.push({
            ...
            run: function (parser) {
                ...
                testTree(parser.configure(__assign({ strict: strict }, config)).parse(text), expected, mayIgnore);
            }
        });
